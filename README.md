...
- 📋 **Worked callsign indicator** (MacLoggerDX only) — each spot row shows a `×N` lifetime QSO count badge and a colored dot if worked today: amber if band/mode unknown, red if band+mode match the spot (likely no new points). Hover for full details. Updates in real time as QSOs are logged.
